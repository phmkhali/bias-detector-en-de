...
Binary Classification means categorizing whether an element belongs into a distinct class, such as "spam" or "not spam". The ML algorithms use information from past examples to create a model or find key rules for making correct decisions. For example, a model can be trained on emails labeled as spam or not spam to detect whether new emails are real. Binary Classification is the most common task in ML % \citep{quemy}. 
\textbf{This thesis attempts to label a translation as " potentially gender biased" or "neutral"}. 

As stated in \autoref{subsection:what_is_gb}, since there is no clear definition on what counts as gender biased, there is no definition on what counts as unbiased. I have defined rules for what counts as biased, but that does not mean that everything not falling into that category is automatically unbiased. Therefore, I have decided on "neutral".

\subsection{Transformer Architecture}
Transformer models are a neural network architecture that incorporate a (self)attention mechanism %\citep{phuong}.
