...
While the previous sections discussed the sources of gender bias, this section focuses on how such biases appear in MT outputs. Drawing on selected studies, I identify recurring patterns of bias and group them into thematic categories to systematically evaluate model behavior.

It is important to note that, due to the complex and multifaceted nature of gender bias, clear boundaries between these categories are not always possible. Definitions in the literature often overlap, and some patterns may extend or intersect with others. The classification presented here reflects my synthesis of the existing research, aiming to provide a structured framework for analysis despite these inherent ambiguities.
...
\addlinespace
Gender Generalisation & Neutral terms assumed to refer to specific gender due to stereotypes & "A researcher must test \textbf{his} model" \\
\addlinespace
Male Default & Masculine form used by default in gender-ambiguous cases & "The doctor" → translated as male, even when gender is unclear \\
\midrule
\multicolumn{3}{l}{\textbf{Contextual Failures}} \\
\midrule
Ignoring Contextual Cues & MT ignores explicit gender markers in source text & "She is a baker" → translated as male baker \\
\addlinespace
Coreference Resolution Failures & MT fails to correctly link pronouns to their referents & "Anna is a teacher. \textbf{She} teaches math." → "teacher" translated as male \\
\bottomrule
\end{tabularx}
\caption{Common gender bias manifestations in MT systems.}
