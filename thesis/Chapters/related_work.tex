...
\label{subsection:manifestations_of_gb}
 ...
 \subsection{Gender Bias in Machine Translation} \label{subsection:definition_gb}
    ...
    We can observe that the translation errors often stem from two main sources: Semantic Bias and Model Overamplification. Both phenomena interact with skewed training data to produce the biased outputs outlined in \autoref{tab:overview_bias_neutral}.

    Semantic Bias arises from the associations the model learns between words and genders. When “he” co‑occurs more often with “doctor,” the model will default to the masculine form in translation \parencite{ullmannGenderBiasMachine2022}. Such defaults lead directly to outputs that either introduce an unwarranted gender or assign the wrong gender, as captured in the first two bias criteria. 
    ...
    They can seem similar because both lead to gendered outputs that are not present in the input. However, the key difference is that semantic bias reflects what the model has learned, while overamplification distorts it.
