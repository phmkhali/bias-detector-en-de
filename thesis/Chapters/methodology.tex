...
\begin{table}[ht]
    \centering
    \begin{tabularx}{\textwidth}{l >{\raggedright\arraybackslash}X rrr}
    \toprule
    \textbf{Dataset} & \textbf{Rationale} & \multicolumn{3}{c}{\textbf{Composition}} \\
    \cmidrule(lr){3-5}
    & & \textbf{Neutral} & \textbf{Biased} & \textbf{Total} \\
    \midrule
    A (Baseline) & Initial setup using equal parts of \texttt{mgente\_final} and \texttt{lardelli\_final}, with some \texttt{tatoeba\_final} neutrals. & 1,750 & 1,500 & 3,250 \\
    B & Built on Baseline. Increased \texttt{lardelli\_final} neutrals to better capture GFL patterns and added more \texttt{tatoeba\_final} neutrals. & 2,150 & 1,500 & 3,650 \\
    C & Built on Baseline and B. Reduced \texttt{lardelli\_final} biased examples to counter possible overrepresentation. & 1,750 & 1,150 & 2,900 \\
    D & Built on Baseline and C. Further improved neutral recognition by adding more \texttt{tatoeba\_final} neutral sentences. & 2,000 & 1,500 & 3,500 \\
    E & Built on Baseline and C. Increased \texttt{mgente\_final} neutral data to raise diversity from naturalistic examples. & 1,750 & 1,500 & 3,250 \\
    \bottomrule
    \end{tabularx}
    \caption{Dataset iterations with rationale and composition. Each version builds on the Baseline and previous adjustments.}
